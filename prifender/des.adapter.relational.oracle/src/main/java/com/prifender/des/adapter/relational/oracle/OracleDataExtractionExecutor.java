...
public class OracleDataExtractionExecutor extends DataExtractionThread {
	...
	private final static String JOB_NAME = "local_project.prifender_oracle_v1_0_1.Prifender_Oracle_v1";
	private final static String DEPENDENCY_JAR = "prifender_oracle_v1_0_1.jar";

	private final OracleDataExtractionETLJobExecutor etlJobUtil;
	private final DataSource ds;
	private int chunkSize;
	private DataExtractionSpec.ScopeEnum scope;
	private final String adapterHome;
	MessagingConnectionFactory messaging;
	EncryptionServiceClient encryptionServiceClient;
	public OracleDataExtractionExecutor(DataSource ds, final DataExtractionSpec spec, final DataExtractionJob job,
		...
		etlJobUtil = new OracleDataExtractionETLJobExecutor(adapterHome);
		this.ds = ds;
	...
	public void run() {
		...
		try( final MessagingConnection messagingServiceConnection = this.messaging.connect() ){
			...
			final String sid = getConnectionParam(ds, "SID");
			...
			int chunkCount = 0;
			 if(this.scope.equals(DataExtractionSpec.ScopeEnum.ALL)){
				   ...
				   int calculatedChunkSize = (this.chunkSize / sampleSizeForChunk ); 
					if(calculatedChunkSize > 1){
						for(int i=0;i<calculatedChunkSize;i++){
							...
							Map<String, String> contextParams = etlJobUtil.getContextParams(jobFilesPath, JOB_NAME, dataSourceUser,
									...
									sid, databaseName, queueName, String.valueOf(offset), String.valueOf(sampleSizeForChunk),String.valueOf(DataExtractionSpec.ScopeEnum.SAMPLE),
					...
					Map<String, String> contextParams = etlJobUtil.getContextParams(jobFilesPath, JOB_NAME, dataSourceUser,
							dataSourcePassword, tableName, dataSourceColumnNames, dataSourceHost, dataSourcePort,
							sid, databaseName, queueName, String.valueOf(offset), String.valueOf(chunkSize), String.valueOf(DataExtractionSpec.ScopeEnum.SAMPLE),
			...
			Map<String, String> contextParams = etlJobUtil.getContextParams(jobFilesPath, JOB_NAME, dataSourceUser,
					dataSourcePassword, tableName, dataSourceColumnNames, dataSourceHost, dataSourcePort,
					sid, databaseName, queueName, String.valueOf(offset), String.valueOf(chunkSize), scope.toString(),
					String.valueOf(chunkSize));
			etlProcess = etlJobUtil.runETLjar(JOB_NAME, DEPENDENCY_JAR, contextParams);
