...
public class CouchBaseDataExtractionExecutor extends DataExtractionThread {
	...
	private CouchBaseDataExtractionETLJobExecutor etlJobUtil;
	...
	public CouchBaseDataExtractionExecutor(DataSource ds, final DataExtractionSpec spec, final DataExtractionJob job,
		...
		etlJobUtil = new CouchBaseDataExtractionETLJobExecutor(adapterHome);
		this.ds = ds;
	...
	public void run() {
		...
		try( final MessagingConnection messagingServiceConnection = this.messaging.connect() ){
			...
			final String dataSourceHost = getConnectionParam(ds, "Host")+":"+getConnectionParam(ds, "Port");
			final String dataSourcePassword = getConnectionParam(ds, "BucketPassword");
			final String queryTimeout = getConnectionParam(ds, "QueryTimeout");
			final String connectTimeout = getConnectionParam(ds, "ConnectTimeout");
				   ...
				   int calculatedChunkSize = (this.chunkSize / sampleSizeForChunk ); 
					if(calculatedChunkSize > 1){
						for(int i=0;i<calculatedChunkSize;i++){
							...
							Map<String, String> contextParams = etlJobUtil.getContextParams(jobFilesPath,
									JOB_NAME, dataSourceTableName, dataSourcePassword, dataSourceTableName,
									dataSourceColumnNames, dataSourceHost, queryTimeout,connectTimeout,
									dataSourceTableName,queueName , String.valueOf(offset),
									String.valueOf(sampleSizeForChunk),String.valueOf(DataExtractionSpec.ScopeEnum.SAMPLE), String.valueOf(sampleSizeForChunk));
							chunkCount++;
					...
					Map<String, String> contextParams = etlJobUtil.getContextParams(jobFilesPath,
							...
							dataSourceColumnNames, dataSourceHost, queryTimeout,connectTimeout,
							dataSourceTableName,queueName , String.valueOf(offset),
							String.valueOf(this.chunkSize), String.valueOf(DataExtractionSpec.ScopeEnum.SAMPLE), String.valueOf(this.chunkSize));
					chunkCount++;
					CommonUtil.postMessage(queue,contextParams,JOB_NAME,this.job.getId(),DEPENDENCY_JAR,scheduleQueueName,ds.getType(),
							1,chunkCount);
