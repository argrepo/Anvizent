...
@Component
public final class RelationalDataSourceAdapter extends DataSourceAdapter
{
    private static final String TYPE_ID = "relational.database.mock";
    
    private static final DataSourceType TYPE = new DataSourceType()
        .id( TYPE_ID )
        .label( "Mock Relational Database" )
    ...
    {
        return RelationalDataSource.metadata();
    }
    ...
    public StartResult startDataExtractionJob( final DataSource ds, final DataExtractionSpec spec, final MessagingConnectionFactory messaging ) throws DataExtractionServiceException
    {
        ...
        if( ! RelationalDataSource.isValidTable( tableName ) )
        {
        ...
        for( final DataExtractionAttribute attribute : spec.getAttributes() )
        {
            ...
            if( ! RelationalDataSource.isValidColumn( tableName, column ) )
            {
        ...
        final RelationalDataExtractionThread thread = new RelationalDataExtractionThread( spec, job, getMockDataSize( ds ), messaging );
