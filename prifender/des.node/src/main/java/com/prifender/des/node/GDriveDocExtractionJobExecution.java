...
public class GDriveDocExtractionJobExecution {
	...
	public List<DocExtractionResult> fetchAndExtractDocContent(Map<String, String> contextParams) throws Exception 
	{
		...
		try {
			...
			Drive service = new Drive.Builder(HTTP_TRANSPORT, JSON_FACTORY, getConnection(userName, serviceAccountName, serviceAccountPrivateKey))
	...
	private String downloadFile(Drive service, FileMetaInfo fileMetaInfo, String downloadFilePath) throws GeneralSecurityException, URISyntaxException, IOException 
	{
		...
		try {
			...
			HttpResponse resp = service.getRequestFactory().buildGetRequest(new GenericUrl(fileMetaInfo.getFileDownloadLink()))
	                    .execute();
	...
	private Credential getConnection(final String userName, final String serviceAccountName, final String serviceAccountPrivateKey) 
