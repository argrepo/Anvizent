...
public class SqlServerDataExtractionExecutor extends DataExtractionThread {
	...
	private static SqlServerDataExtractionETLJobExecutor etlJobUtil;
	...
	public SqlServerDataExtractionExecutor(DataSource ds, final DataExtractionSpec spec, final DataExtractionJob job,
		...
		etlJobUtil = new SqlServerDataExtractionETLJobExecutor(adapterHome);
		this.ds = ds;
	...
	public void run() {
		...
		try( final MessagingConnection messagingServiceConnection = this.messaging.connect() ){
			...
			final String jobFilesPath = this.adapterHome;
			...
			int chunkCount = 0;
			 if(this.scope.equals(DataExtractionSpec.ScopeEnum.ALL)){
				   ...
				   int calculatedChunkSize = (this.chunkSize / sampleSizeForChunk ); 
					if(calculatedChunkSize > 1){
						for(int i=0;i<calculatedChunkSize;i++){
							...
							Map<String, String> contextParams = etlJobUtil.getContextParams(jobFilesPath, JOB_NAME, dataSourceUser,
									dataSourcePassword, dataSourceTableName, dataSourceColumnNames, dataSourceHost, dataSourcePort,
									databaseName, instanceName, queueName, String.valueOf(offset), String.valueOf(sampleSizeForChunk),
					...
					Map<String, String> contextParams = etlJobUtil.getContextParams(jobFilesPath, JOB_NAME, dataSourceUser,
							dataSourcePassword, dataSourceTableName, dataSourceColumnNames, dataSourceHost, dataSourcePort,
							databaseName, instanceName, queueName, String.valueOf(offset), String.valueOf(this.chunkSize),
		...
		/*	int offset = 0;
			Map<String, String> contextParams = etlJobUtil.getContextParams(jobFilesPath, JOB_NAME, dataSourceUser,
					dataSourcePassword, dataSourceTableName, dataSourceColumnNames, dataSourceHost, dataSourcePort,
					databaseName, instanceName, queueName, String.valueOf(offset), String.valueOf(chunkSize),
					this.scope.toString(), String.valueOf(chunkSize));
